	...
	public static void main(String[] args) {
		...
		while (check = true) {
			...
			case "1":
				...
				javax.persistence.Query query = manager.createQuery(findAll);
				List<StudentDetails> details = query.getResultList();
				for (StudentDetails studentDetails : details) {
			...
			case "2":
				...
				if (5 >= rollno1) {
					try {
						throw new InvalidRollNoException("id not present in the database");
					} catch (Exception e) {
					...
					javax.persistence.Query query2 = manager.createQuery(findAny);
					query2.setParameter("rollno", rollno1);
					StudentDetails studentDetails = (StudentDetails) query2.getSingleResult();
					System.out.println(studentDetails);
				}
			...
			case "3":
				...
				case "y":
					...
					javax.persistence.Query query3 = manager.createQuery(update);
